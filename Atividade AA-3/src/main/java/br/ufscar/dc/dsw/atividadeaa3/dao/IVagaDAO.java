...
public interface IVagaDAO extends JpaRepository<Vaga, Long> {
    ...
    @Query("select v from Vaga v where v.empresa.id = :idEmpresa and v.dataLimite >= :dataLimite")
    List<Vaga> searchByEmpresaIdAndDataLimite(@PathVariable("idEmpresa") Long idEmpresa, @PathVariable("dataLimite") Date dataLimite);
